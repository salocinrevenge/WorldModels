...
class TemporalEncoding(nn.Module):
    ...
    def forward(self, x):
        ...
        if ("cpu" not in str(x.device)):
            # print("em networks 173 x: ",x.shape, " arange: ", torch.arange(x.shape[0]).cuda().shape)
            # print(f"{self.pos_enc(torch.arange(x.shape[0]).cuda()).unsqueeze(1).shape=}")
            x_pos = x.unsqueeze(1) + self.pos_enc(torch.arange(x.shape[0]).cuda()).unsqueeze(1) #[S,1,D]
            # print(f"{self.cls_token(torch.tensor([0]).cuda()).unsqueeze(0).shape=} {x_pos.shape=}, {x_pos=}, networks 175")
            x_cls = torch.cat([self.cls_token(torch.tensor([0]).cuda()).unsqueeze(0), x_pos], dim=0) #[S+1,1,D]
        else:
            self.pos_enc = self.pos_enc.to('cpu')
            self.cls_token = self.cls_token.to('cpu')
            # print("em networks x: ",x.shape, " arange: ", torch.arange(x.shape[0], device='cpu'))
            x_pos = x.unsqueeze(1) + self.pos_enc(torch.arange(x.shape[0], device='cpu')).unsqueeze(1) #[S,1,D]
            # print(f"{self.cls_token(torch.tensor([0], device='cpu')).unsqueeze(0).shape=} {x_pos.shape=}")
            x_cls = torch.cat([self.cls_token(torch.tensor([0], device='cpu')).unsqueeze(0), x_pos], dim=0) #[S+1,1,D]
...
class HierarchicalPrediction(nn.Module):
    ...
    def forward(self, x):
        ...
        if ("cpu" not in str(x.device)):
            ...
            self.pos_enc.to('cpu')
            reps_pos = x.unsqueeze(1) + self.pos_enc(torch.arange(x.shape[0], device='cpu')).unsqueeze(1)
            self.network = self.network.to('cpu')
