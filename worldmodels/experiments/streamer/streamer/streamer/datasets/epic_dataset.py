...
class EpicDataset(Dataset):
    ...
    def __init__(self, args: EpicDatasetArguments):
        ...
        vids_map_path = os.path.join(args.dataset, 'videos/map.json')
        ...
        self.vids_paths = dict(random.sample(list(self.vids_paths.items()), int(len(self.vids_paths)*args.percentage/100.0)))
        print(f'processing {len(self.vids_paths)} for the {args.split} split: {self.vids_paths}')
